...
def _process_chunk_inline(chunk_data, classifier):
    ...
    # Для скорости не формируем полный список строк результата.
    # Excel-вывод является управленческим отчетом, а не построчной выгрузкой.
    # Для интерфейса оставляем только небольшой preview.
    processed_rows = []
    preview_limit = 1000
    chunk_stats = {
        "_rows_count": n,
...
def run_pipeline(input_path: str, output_path: str, use_llm: bool = False,
    ...
    def _write_results(processed_rows, chunk_stats):
        ...
        if len(preview_rows) < 1000:
            preview_rows.extend(processed_rows[:1000 - len(preview_rows)])
        # processed_rows содержит только preview, поэтому счетчик берем из статистики чанка.
        total_count += int(chunk_stats.get("_rows_count", len(processed_rows)))
    ...
    for district, d_stats in sorted_districts[:3]:
        ...
        if use_llm:
            llm_summary = generate_district_summary_llm(
                district,
                d_stats["summaries"],
                ollama_url=ollama_url,
                source_context=ollama_context,
            )
            if llm_summary:
    ...
    # Фактическое время может быть добавлено вызывающим кодом, но режим фиксируем здесь.
    stats["processing_mode"] = "fast_hybrid"
    stats["llm_mode"] = "Ollama используется для контекста и итоговых объяснений, не для каждой строки"
