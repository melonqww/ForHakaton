...
_FILLER_STARTS_TUPLE = (
    ...
    "жители крайне возмущены", "proсим обратить",
...
def extract_summary_local(text: str, max_chars: int = 300, text_lower: str = None) -> str:
    ...
    if "переезд" in t_lower:
        ...
        if any(w in t_lower for w in ["серый", "яма", "разбит", "плохой"]):
    ...
    # Ямы / Дороги
    if "яма" in t_lower or "дорог" in t_lower or "выбоин" in t_lower or "исфальт" in t_lower or "колея" in t_lower or "асфальт" in t_lower:
